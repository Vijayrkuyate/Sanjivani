...
namespace SanjivaniERP.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            //OpcCommunication.ConnectOPCServer();
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if(userm==null)
            {
                Session["Loginmsg"] = "Invalid User Name And Password Please Try Again";
                return View(model);
            }
           ...
           var result = await SignInManager.PasswordSignInAsync(userm.Email, model.Password, model.RememberMe, shouldLockout: false);
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    Session["ProfilePhoto"] = Convert.ToString(dt.Rows[0]["ProfilePhoto"]);
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> _PartialCustomerRegister(FormCollection fc, CPCchannelPartnerModel model, HttpPostedFileBase[] postedFile)
        {
            if (ModelState.IsValid)
            {
                int custid = Convert.ToInt32(model.CustId);
                ...
                else
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        {
                            model.AspUserId = UserId;
                            model.ParentId = Session["UserId"].ToString();
                            model.CustCategroryId = "4";
                            var EventsTitleList = objPartnerBAL._partialCPCSave(model);
                            if (Convert.ToInt32(EventsTitleList) > 0)
                            {
                                Session["Tab"] = "2";
                                ...
                                Session["Msg"] = "SaveData";
                                model.CustId = Convert.ToString(Session["CustId"]);
                                return RedirectToAction("AddNewCustomer", "Partner", new { CustId = model.CustId });
                            }
                        }
                    }
                }
      ...
      //  [HttpPost]
       // [ValidateAntiForgeryToken]
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> _PartialEmployeeRegister(FormCollection fc, Employees model, HttpPostedFileBase[] postedFile)
        {
            if (ModelState.IsValid)
            {
                if (string.IsNullOrWhiteSpace(model.CustId)|| model.CustId=="0")
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        // if (ModelState.IsValid)
                        {
                            model.AspUserId = UserId;
                            model.ParentId = "1";
                            model.CustCategroryId = "7";
                            var EventsTitleList = objPartnerBAL._partialEmpSave(model);
                            if (Convert.ToInt32(EventsTitleList) > 0)
                            {
                                Session["Tab"] = "1";
                                Session["CustId"] = EventsTitleList;
